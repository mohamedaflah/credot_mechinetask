...
import { updateVarientAction } from "@/redux/actions/product/Varients/updateVarient";

export function UpdateVarient() {
  ...
  useEffect(() => {
    setValue("color", String(selectedVarient?.color));
    setValue("description", String(selectedVarient?.description));
    setValue("stock", Number(selectedVarient?.stock));
    setValue("price", Number(selectedVarient?.price));
    setValue("modelNumber", String(selectedVarient?.modelNumber));
    if (selectedVarient && selectedVarient.releasedDate) {
      setValue("releasedDate", String(selectedVarient?.releasedDate));
    }

    setValue("specifications", [...(selectedVarient?.specifications || [])]);
    const fetchImages = async () => {
      if (selectedVarient && selectedVarient.images) {
        const imagePromises = selectedVarient.images.map((image) =>
          imageUrlToFileObject(image)
        );

        try {
          const imageFiles = await Promise.all(imagePromises);
          setValue("images", imageFiles as unknown as (FileList | null)[]);
        } catch (error) {
    ...
    fetchImages();
    setValue("status", String(selectedVarient?.status));
    if (selectedVarient?.memory) {
      setValue("memory", selectedVarient?.memory);
    }
  }, [selectedVarient, setValue]);
  ...
  const submitVarientEditForm = (values: z.infer<typeof productSchema>) => {
    if (
      values.category &&
  ...
  return (
    <main className="w-full h-full overflow-y-auto">
      ...
      <form
        ...
        onSubmit={handleSubmit(submitVarientEditForm)}
      >
            ...
            <div className="w-full h-full overflow-auto p-2 space-y-3">
              <div className="space-y-1 w-full">
                ...
                <Select onValueChange={(value) => setValue("memory", value)}>
                  <SelectTrigger className="w-full md:w-full">
                    <SelectValue placeholder={watch("memory")} />
                  </SelectTrigger>
                  <SelectContent>
